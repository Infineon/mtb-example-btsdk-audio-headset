...
#define MAX_NUM_OF_SIMULTANEOUS_AVRCP_CONNECTIONS   2
...
static tRC_APP_CB rc_app_cb[MAX_NUM_OF_SIMULTANEOUS_AVRCP_CONNECTIONS];
extern wiced_timer_t avrc_timer;
...
static uint16_t get_connection_handle_from_address(wiced_bt_device_address_t bd_addr)
{
    ...
    for( ; i< MAX_NUM_OF_SIMULTANEOUS_AVRCP_CONNECTIONS; i++)
    {
        ...
        if( rc_app_cb[i].connection_state == REMOTE_CONTROL_DISCONNECTED && idle_index == 0xFFFF)
        {
            idle_index = i;
        }
    }
...
wiced_result_t avrc_app_hci_pass_through (uint16_t handle, uint8_t op_id, uint8_t state  )
{
    ...
    if( idx >= MAX_NUM_OF_SIMULTANEOUS_AVRCP_CONNECTIONS )
    {
        WICED_BT_TRACE("%s Bad Index: %d Handle: %d\n\r", __FUNCTION__ , idx, handle);
        return WICED_BADARG;
    }

    ptr_cb = &rc_app_cb[idx];

    WICED_BT_TRACE("%s op_id:%d state:%d\n",__FUNCTION__, op_id, state);
...
wiced_result_t avrcp_app_hci_set_player_settings ( uint16_t handle, uint8_t attr_id )
{
    ...
    if ( ptr_cb->connection_state == REMOTE_CONTROL_CONNECTED )
    {
        ...
        ptr_cb->app_setting[attr_id].current_index =
                        (ptr_cb->app_setting[attr_id].current_index + 1) %
                         ptr_cb->app_setting[attr_id].num_possible_values;

        app_param.attr_value[0] =
                  ptr_cb->app_setting[attr_id].possible_values[ptr_cb->app_setting[attr_id].current_index];
...
wiced_result_t avrc_app_hci_disconnect_connection ( uint16_t handle )
{
    ...
    if ( ptr_cb->connection_state == REMOTE_CONTROL_CONNECTED )
    {
        WICED_BT_TRACE( "%s: Disconnecting AVRCP for Handle: %d\n", __FUNCTION__,  handle);
...
wiced_result_t avrcp_app_hci_control_volume ( uint8_t* p_data, uint32_t len)
{
    wiced_result_t result = WICED_NOT_CONNECTED;
    uint16_t handle = p_data[0] | (p_data[1] << 8);
    uint8_t  volume = p_data[2] * MAX_AVRCP_VOLUME_LEVEL / 100;
    tRC_APP_CB* ptr_cb;
    uint16_t idx;
    ...
    if ( ptr_cb->connection_state == REMOTE_CONTROL_CONNECTED )
    {
        result = wiced_bt_avrc_ct_set_volume_cmd(handle, volume);
    }
...
wiced_result_t avrcp_app_hci_get_play_status(uint8_t handle)
{
    wiced_result_t result = WICED_ERROR;
    uint16_t idx;
    tRC_APP_CB* ptr_cb;
    ...
    if ( ptr_cb->connection_state == REMOTE_CONTROL_CONNECTED )
    {
        result = wiced_bt_avrc_ct_get_play_status_cmd(handle);
    }
                                  ...
                                  uint32_t peer_features)
{
    ...
    switch( connection_state )
    {
        ...
        {
            idx = get_connection_handle_from_address(remote_addr);
            if( idx >= MAX_NUM_OF_SIMULTANEOUS_AVRCP_CONNECTIONS )
            {
                WICED_BT_TRACE("%s Bad Index: %d Handle: %d\n\r", __FUNCTION__ , idx, handle);
                return;
...
#endif
            if ( wiced_is_timer_in_use(&avrc_timer) )
            {
                WICED_BT_TRACE("stopping timer\n");
                wiced_stop_timer( &avrc_timer );
            }
            app_state_cb.avrc_state = 1;
...
void avrc_handle_track_change_event(uint8_t handle)
{
    wiced_result_t status =
        wiced_bt_avrc_ct_get_element_attr_cmd(handle, 0,
 ...
 * @return          Nothing
 */
void avrc_handle_element_attribute_rsp(uint8_t handle, wiced_bt_avrc_response_t *avrc_rsp)
{
    int i, j;
    int rsp_size;
    uint8_t *rsp;
...
void avrc_send_app_setting_info(uint8_t handle)
{
    ...
    {
        for (k=0; k<ptr_cb->app_setting[i].num_possible_values; k++)
        {
            rsp[j] |= 1 << ptr_cb->app_setting[i].possible_values[k];
        }
    }
    ...
    {
        if (ptr_cb->app_setting[i].available)
        {
                                               ...
                                               wiced_bt_avrc_response_t *avrc_rsp)
{
    ...
    ptr_cb->num_app_settings = ptr_cb->num_app_settings_init = 0;
    if (list_app_attr->num_attr <= MAX_POSSIBLE_APP_ATTR_SETTINGS)
    {
        for (i = 0; i < list_app_attr->num_attr; i++)
        {
            ...
            if (list_app_attr->attrs[i] <= MAX_POSSIBLE_APP_ATTR_SETTINGS)
            {
                ...
                ptr_cb->app_setting[list_app_attr->attrs[i]].available = TRUE;
                ptr_cb->num_app_settings++;
            }
        }
        ...
        for ( i = 0; i < list_app_attr->num_attr; i++ )
        {
            ...
            result = wiced_bt_avrc_ct_list_player_values_cmd( handle, list_app_attr->attrs[i] );
            if ( result != WICED_SUCCESS )
            {
                break;
            }
                                           ...
                                           wiced_bt_avrc_response_t *avrc_rsp)
{
    ...
    ptr_cb->num_app_settings_init++;

    /* Cache the possible values for the player app attribute */
    ptr_cb->app_setting[attr_id].num_possible_values = list_app_values->num_val;
    for ( i = 0; i < list_app_values->num_val; i++ )
    {
        ptr_cb->app_setting[attr_id].possible_values[i] = list_app_values->vals[i];
    }

    /* Check if all app possible setting requests have completed */
    if (ptr_cb->num_app_settings_init == ptr_cb->num_app_settings)
    {
                                           ...
                                           wiced_bt_avrc_response_t *avrc_rsp)
{
    ...
    if (get_cur_app_val->num_val == 1)
    {
        ...
        for ( i = 0; i <= ptr_cb->app_setting[attr_id].num_possible_values; i++ )
        {
            if ( attr_val == ptr_cb->app_setting[attr_id].possible_values[i] )
            {
                uint8_t rsp[5] = {0, 0, 1, 0, 0};
                rsp[0] = handle;
                ptr_cb->app_setting[attr_id].current_index = i;
...
/**
 *
 * Function         avrc_handle_get_play_status_rsp
 *
 *                  Callback invoked by avrc_response_cback to inform of a response to the
...
void avrc_handle_get_play_status_rsp(uint8_t handle, wiced_bt_avrc_response_t *avrc_rsp)
{
    ...
    hci_control_avrc_send_play_status_info(handle, get_play_status_val->play_status, get_play_status_val->song_len, get_play_status_val->song_pos);
}
...
{
    WICED_BT_TRACE("AVRC Command Callback :%d\n", avrc_cmd->pdu);
    switch( avrc_cmd->pdu )
    {
        case AVRC_PDU_SET_ABSOLUTE_VOLUME:
            hci_control_send_avrc_volume(handle, (uint8_t)((avrc_cmd->volume.volume * 100) / MAX_AVRCP_VOLUME_LEVEL));
        break;
...
/**
 *
 * Function         headset_remote_control_init
 *
 ...
 * @return          Nothing
 */
void headset_remote_control_init( void )
{
...
uint8_t hci_control_avrc_handle_ctrlr_command( uint16_t cmd_opcode, uint8_t *p_data, uint16_t payload_len )
{
    ...
    case HCI_CONTROL_AVRC_CONTROLLER_COMMAND_GET_TRACK_INFO:               /* Get Track Metadata */
        ...
        status = avrcp_app_hci_get_attrs_cmd(p_data, payload_len) == WICED_SUCCESS ? HCI_CONTROL_STATUS_SUCCESS : HCI_CONTROL_STATUS_FAILED;
        break;
    ...
    case HCI_CONTROL_AVRC_CONTROLLER_COMMAND_VOLUME_LEVEL:
        status = avrcp_app_hci_control_volume( p_data, payload_len );
        break;

    case HCI_CONTROL_AVRC_CONTROLLER_COMMAND_GET_PLAY_STATUS:
        status = avrcp_app_hci_get_play_status( handle );
        break;
    default:
