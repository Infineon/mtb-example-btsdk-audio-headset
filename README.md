...
## Overview
This is a Headset reference application. It includes support for Audio Sink, Hands-free, and AVRC profiles.

Features demonstrated:

 - WICED BT AV (A2DP/AVRCP) APIs
 - WICED BT GATT APIs (implementation of Vendor Specific Device)
 - Handling of the UART WICED HCI protocol
 - SDP and GATT descriptor/attribute configuration

## Instructions
This application can run as a standalone Headset or in association with ClientControl. When running in standalone mode, it is required that the 20706 controls the codec chip. Currently this is not implemented. In association with the ClientControl app (providing apps processor/MCU emulation), this app serves to abstract the details of Bluetooth protocols and profiles while allowing ClientControl to deal with the business logic. ClientControl is typically connected over UART and can send commands and receive notifications.
...
To use this application in conjunction with ClientControl:
 ...
 - Build and download the application image directly using the IDE or command line.
 - OR: to build a downloadable hcd file to use to download through ClientControl, uncomment 'DIRECT\_LOAD=1' in the application makefile. The HCD file thus generated can be downloaded into 20706 RAM by ClientControl (instead of NVRAM with the normal program operation from the IDE or command line).
 - Ensure that the STANDALONE\_HEADSET\_APP flag is disabled.

To use this in stand alone mode:

 - Ensure that the STANDALONE\_HEADSET\_APP flag is enabled.
...
The application also demonstrates Vendor Specific Device (hello sensor) that processes write requests from the client.
